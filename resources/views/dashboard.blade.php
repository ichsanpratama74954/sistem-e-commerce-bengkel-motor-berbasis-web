@php
    ...
    // Normalisasi role: Mengubah string role dari DB (misal: Admin / Pelanggan) ke huruf kecil semua agar pengecekan 100% akurat
    $userRole = strtolower(trim(auth()->user()->role ?? ''));
@endphp

@component('layouts.app')
    <div class="space-y-6">
        {{-- BAGIAN HEADER DAHSBOARD --}}
        <div>
            <flux:heading size="xl" level="1" class="font-black text-indigo-950 dark:text-indigo-400">
                Selamat Datang di Dashboard Bengkel 🛠️
            </flux:heading>
            
            @if($userRole === 'pelanggan')
                <flux:subheading size="sm" class="text-zinc-500 dark:text-zinc-400">
                    Kemudahan memantau status servis dan kendaraan kesayangan Anda.
                </flux:subheading>
            @else
                <flux:subheading size="sm" class="text-zinc-500 dark:text-zinc-400">
                    Pantau performa bisnis dan pesanan masuk hari ini.
                </flux:subheading>
            @endif
        </div>

        {{-- 1. BLOK KARTU STATISTIK (HANYA UNTUK ADMIN & MEKANIK)--}}
        ...
        {{-- 2. BLOK AKSI CEPAT OPERASIONAL (UNTUK ADMIN & MEKANIK)--}}
        @if(in_array($userRole, ['admin', 'mekanik']))
            <div class="p-6 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800">
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    {{-- Tombol Operasional yang bisa diakses Admin & Mekanik --}}
                    ...
                    {{-- Fitur khusus Finansial & Inti (HANYA UNTUK ADMIN, Mekanik tidak bisa lihat) --}}
                    @if($userRole === 'admin')
                        ...
                        <a href="{{ route('reports.index') }}"
                            wire:navigate
                            class="p-4 rounded-lg border border-blue-200 bg-blue-50 hover:bg-blue-100 transition rounded-xl text-center">
                                <div class="text-3xl mb-2">📊</div>
                                <div class="font-semibold">
                                    Laporan Penjualan
                                </div>
                                <div class="text-sm text-gray-500">
                                    Lihat statistik & ekspor PDF
                                </div>
                        </a>
                    @endif
                </div>
        ...
        {{-- 3. BLOK NAVIGASI MANDIRI (HANYA UNTUK ROLE PELANGGAN)               --}}
        @if($userRole === 'pelanggan')
            <div class="p-6 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800">
                <flux:heading size="lg" class="mb-2">Menu Layanan Pelanggan</flux:heading>
                <flux:subheading size="sm" class="mb-6">Silakan pilih opsi di bawah ini untuk melihat data atau melakukan pemesanan servis.</flux:subheading>
                
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <a href="/motorcycles" wire:navigate class="p-6 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition block text-center">
                        <div class="text-4xl mb-3">🏍️</div>
                        <div class="font-bold text-zinc-900 dark:text-white text-lg">Motor Saya</div>
                        <div class="text-sm text-zinc-500 mt-1">Daftarkan dan cek kondisi motor Anda</div>
                    </a>
                    <a href="/bookings" wire:navigate class="p-6 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition block text-center">
                        <div class="text-4xl mb-3">📅</div>
                        <div class="font-bold text-zinc-900 dark:text-white text-lg">Booking Servis Baru</div>
                        <div class="text-sm text-zinc-500 mt-1">Atur jadwal kunjungan servis secara online</div>
                    </a>
                    <a href="/orders" wire:navigate class="p-6 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition block text-center">
                        <div class="text-4xl mb-3">📜</div>
                        <div class="font-bold text-zinc-900 dark:text-white text-lg">Riwayat Transaksi</div>
                        <div class="text-sm text-zinc-500 mt-1">Lihat nota pembayaran dan status pesanan</div>
                    </a>
                </div>
            </div>
        @endif
